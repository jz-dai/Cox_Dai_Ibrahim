...
```{r}
# importing dataset
microplastics_byyear <- read.csv(here("NPSG_Microplastics_Processed/microplastics_data_wrangling.csv"))
```
...
```{r}
# setting date as date format and selecting year in between the period of interest
...
leaflet(data = period1.sf) %>%
  ...
  addCircleMarkers(
    ...
    color = ~density_palette(Density.Class), # Color based on Density.Class
    popup = ~popup, # Add popups
    fillOpacity = 0.8
  ) %>%
  addLegend(
    "bottomright", 
...
# Define a color palette matching the new order
density_palette <- colorFactor(
  palette = c("green", "yellow", "orange", "red", "purple"), # Choose colors for each class
  levels = levels(period1.sf$Density.Class) # Uses the reordered levels
)
