...
We determined that the columns such as references, organization, and DOI were not useful in this study. We thus wrangled the dataset to select only the columns of interest, such as sampling method, measurement, unit, density range, and x-y coordinates.  

The dataset contains multiple sampling methods, as discussed in the introduction. This creates challenges in the comparing data with different measurement units. For the purpose of this study, we sought to identify unique units of measurement, and focus on one in our analysis.
...
Hand picking, grab sample, and PVC cylinder had relatively narrow boxes, suggesting lower variability in measurements. Outliers could be observed in methods such as Neuston net and grab sample, indicating occasional extreme values in these datasets.  
